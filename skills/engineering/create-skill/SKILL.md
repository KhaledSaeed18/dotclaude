---
name: create-skill
description: Author a new skill for this repository end to end — choose its category, write trigger-friendly frontmatter, structure the SKILL.md, split reference material into companion files, then regenerate the registry and README catalog. Use when creating, scaffolding, restructuring, or reviewing a skill in this repo.
...
This repo's source layout is `skills/<category>/<name>/SKILL.md`. The **category is the folder** — there is no category field. Everything else (per-skill `registry.json`, the root `registry.json`, the README catalog) is *generated* by `pnpm gen` from the folder location plus the frontmatter. Never hand-edit generated files.
...
- **The `SKILL.md` body is always loaded when the skill triggers** — keep it lean. Push reference material, long tables, examples, and templates into companion files the agent reads on demand (progressive disclosure).
...
- Hard rules and steps are concrete and safe — confirmations on anything irreversible or outward-facing.
