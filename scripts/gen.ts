...
interface Attribution {
  path: string;
  /** `adapted`: material reused under a permissive licence; `inspired`: a shape or idea, nothing copied. */
  kind: "adapted" | "inspired";
  text: string;
}

function buildNotice(attributions: Attribution[]): string {
  const lines = [
    "# Notice",
    "",
  ...
  const adapted = attributions.filter((a) => a.kind === "adapted");
  const inspired = attributions.filter((a) => a.kind === "inspired");
  if (adapted.length === 0) lines.push("_None._", "");
  for (const { path, text } of adapted) lines.push(`### ${path}`, "", text, "");
  lines.push(
    "## Inspired by",
    "",
  ...
  if (inspired.length === 0) lines.push("_None._", "");
  for (const { path, text } of inspired) lines.push(`### ${path}`, "", text, "");
  return lines.join("\n");
}
...
function generate(): GeneratedFile[] {
  ...
  for (const ct of CONTENT_TYPES) {
    ...
    for (const item of items(ct)) {
      ...
      const attribution = manifestText.match(/^## Attribution\s*\n([\s\S]*?)(?=^## |\s*$)/m);
      if (attribution?.[1]) {
        attributions.push({ path: toPosix(itemDir), kind: "adapted", text: attribution[1].trim() });
      }
      const inspired = manifestText.match(/^## Inspired by\s*\n([\s\S]*?)(?=^## |\s*$)/m);
      if (inspired?.[1]) {
        attributions.push({ path: toPosix(itemDir), kind: "inspired", text: inspired[1].trim() });
      }
