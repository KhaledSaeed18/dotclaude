...
Skills that bundle companion files (like `grill-with-docs`) install a `docs/` subdirectory alongside `SKILL.md`, and the same `cp -R` command handles that automatically.

## Catalog

The catalog below lists every skill in this repository, grouped by category.
