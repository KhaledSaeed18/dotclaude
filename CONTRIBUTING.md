...
**Adapting from other projects.** Only MIT, Apache-2.0, CC0, CC-BY, BSD, or ISC sources. The manifest ends with an `## Attribution` section naming the source (as a link) and its licence; `pnpm gen` collects these into `NOTICE.md` and `pnpm validate` rejects a section without both. Rewrite in this repository's voice; attribution covers borrowed structure and substance, not pasted text. When an item only follows the *shape* of something you may not copy (a role, a scope, a pipeline order; typically from a source that is not open-licensed), end it with `## Inspired by` naming the source as a link and stating that nothing was copied; validate requires the link, and `NOTICE.md` lists these separately.
